...
- push 전에는 tree 전체에서 실제 env 값, bearer token, API key, webhook URL, private key material을 스캔합니다. 문서에는 `<secret>`, `<discord-bot-token>`, `test-token` 같은 placeholder만 남깁니다.
