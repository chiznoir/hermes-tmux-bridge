...
`hermes-codex-bridge`는 **bridge core**입니다. Hermes가 로컬 Codex 세션을 다룰 수 있도록 작은 HTTP service와 helper CLI를 제공합니다. 로컬 log와 tmux에서 세션을 찾고, Codex 답변 원문을 읽고, visible tmux pane에 후속 명령을 전달하며, 선택된 이벤트를 Hermes Gateway를 통해 Discord로 보냅니다.

```text
Hermes / Discord
  -> 127.0.0.1에서 실행되는 hermes-codex-bridge
...
Hermes가 개발자와 같은 로컬 근거를 볼 수 있으면 작업을 더 안정적으로 오케스트레이션할 수 있습니다. 이 bridge는 Codex lifecycle log, Codex JSONL transcript, tmux pane, command/audit history를 localhost 안에서 읽고 Hermes에 필요한 좁은 API만 제공합니다.
