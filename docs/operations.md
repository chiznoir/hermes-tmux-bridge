...
# OMX helper default: omx --madmax --high
```
...
`omx-new`, `omx-send`, `omx-kill`은 repository의 `bin/`에 있습니다. 설치/갱신은 아래 명령을 사용합니다.
...
```text
omx-new [PROJECT_DIR] [--name SESSION] [--attach] [--json] [--no-check] [-- OMX_ARGS...]
omx-send --session SESSION_ID [--dry|--dry-run] [--hold|--no-submit] '<prompt>'
omx-send --session <bridgeSessionId> --discord-approval '<prompt>'
omx-kill --session SESSION_ID
```

Hermes가 visible session을 만들 때는 raw `omx`/`codex` 직접 실행 대신 `omx-new`를 사용합니다. `omx-new`는 `omx --madmax --high` 기반 tmux session을 만들고, OMX lifecycle metadata와 Codex JSONL log를 bridge가 관측하게 합니다. 운영 규칙에서 raw tmux paste나 직접 `codex` 실행을 기본 경로처럼 추가하지 않습니다.
...
- `/new`와 `/resume`은 기존 OMX/Codex pane에 전달되는 Codex slash command입니다. 기존 pane에 전달하는 경우 `omx-send`가 원문 의미를 보존해 그대로 보냅니다.
...
- `omx-send --session <id> --answer-approval send|reject --question-id <questionId>`로 answer가 들어가는지 확인합니다.
