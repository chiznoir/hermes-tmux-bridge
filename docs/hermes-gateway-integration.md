# Hermes Gateway ↔ hermes-codex-bridge 연결 가이드

이 문서는 Hermes Gateway와 `hermes-codex-bridge`를 연결하는 운영 가이드입니다. 빠른 설치는 [`docs/quickstart-ko.md`](quickstart-ko.md)를 먼저 보세요.
...
Discord user
  ...
  -> hermes-codex-bridge skill
  -> hermes-codex-bridge HTTP API
  -> Codex/tmux session
```
...
```text
hermes-codex-bridge event router
  -> Hermes webhook sink
  -> POST /webhooks/codex-bridge
...
# 일반 설치는 live tmux Codex project root를 자동 발견한다.
# PROJECT_ROOT=/path/to/fixed-codex-project

# 로컬-only면 unset. LAN/Docker/public이면 필수.
# BRIDGE_TOKEN=<random-long-token>
```

생략하는 기본값: `HOST=127.0.0.1`, `PORT=3037`, `BRIDGE_PUBLIC_URL=http://127.0.0.1:3037`, `BRIDGE_HERMES_PROJECT_CHANNEL_MAP=~/.config/hermes-codex-bridge/project-channels.json`, `BRIDGE_HERMES_NOTIFICATION_MODE=direct`.
...
scripts/install-systemd-service.sh \
  ...
  --sink-secret-file ~/.config/hermes-codex-bridge/hermes-webhook.secret \
  --channel <fallback-channel-id> \
  --map ~/.config/hermes-codex-bridge/project-channels.json
```
...
Hermes는 `hermes-codex-bridge` skill을 통해 다음을 수행합니다.

- `codex-send --list`로 세션 목록 확인
...
- `codex-send --session <id>`로 후속 지시 전달
- Discord에서 정제 프롬프트 전송 전 operator 승인이 필요하면 `codex-send --session <id> --discord-approval` 사용
- `codex-new`로 새 visible Codex 세션 시작
- `codex-kill`로 Codex/tmux 세션 종료

Trigger 문구는 일반 대화가 아니라 도구 호출로 라우팅합니다. 알림 reply의 “이 세션/이거/방금 알림”은 reply payload의 `bridge_session_id`를 우선하며 project 최신 세션으로 추정하지 않습니다. reply에 `전달`/`보내`/`넘겨`/`세션에 넣어`가 들어가면 설명/조회보다 `codex-send --session <bridge_session_id>`가 최우선입니다. Discord-originated Hermes 전달에서 operator가 정제 프롬프트를 먼저 보고 전송/거절/추가수정을 선택해야 하면 `--discord-approval`을 붙인 뒤 Hermes `clarify`/AskUserQuestion으로 실제 Discord 승인 카드를 띄웁니다. 반영/수정/재시도/진행/계속/위 내용/답장 관련 reply(예: “답장하기”, “위 내용 전달”, “이거 반영”, “답장으로 보낸 거”, “방금 알림에 대한 말”)도 `전달해`가 없어도 `codex-send`; “이건 뭐냐”, “궁금한데” 같은 단순 설명 요청은 dispatch 단어가 없을 때만 Hermes가 read endpoint로 확인해 설명; reply의 “종료/kill/킬/죽여”는 `codex-kill --session <bridge_session_id>`; “원문 알려줘”, “마지막 답변 원문”, “raw”, “full text”는 `/sessions/:id/idle/latest` `fullText`; “세션 열려있어?”, “세션 살아있어?”, “현재 세션 확인”은 `codex-send --list` 또는 `/sessions/:id/state`; “최근 로그/이벤트”는 `/sessions/:id/events`; “내가 보낸 이력/명령 이력”은 `/sessions/:id/interactions`; 사용자가 새 visible 세션을 직접 요청한 “새 세션”, “세션 열어”, “시작해”만 `codex-new`를 사용합니다. bridge webhook `SessionStart` 알림 본문은 새 세션 트리거가 아닙니다. `/new`/`/resume`은 Hermes/Gateway 명령이 아니라 Codex 입력 slash command로 구분합니다. 기존 Codex tmux pane에 보내는 `/new`/`/resume`은 `codex-send --session <bridge_session_id>`로 원문 그대로 전달하고, bridge가 이후 새 Codex session id를 감지해 기존 tmux/bridge session mapping을 새 Codex mapping으로 따라가게 해야 합니다. 일반 승인/거절은 AskPermission이 실제로 있을 때만 다룹니다.

처음 바인딩한 세션에는 `codex-send --dry` 또는 `codex-send --dry-run`을 먼저 보내고, live tmux 화면에 보여야 하는 지시는 `codex-send --mode tmux`를 사용합니다. 세션 시작/전달/종료 작업은 Hermes가 터미널에서 bridge API용 `curl -X POST ...`를 직접 만들지 않고 위 helper CLI를 사용해야 합니다. `codex-send` 성공 후 사용자에게 보고할 때는 프롬프트 원문을 별도 확인 메시지로 재출력하지 않습니다. 입력 원문은 bridge의 `CommandSubmitted`/`User Command` 이벤트 알림으로 남습니다.

Helper CLI는 이 repository의 `bin/` 기준입니다. `scripts/install-hermes-stack.sh`가 기본으로 `codex-new`/`codex-send`/`codex-kill`을 `PATH`에 설치하며, CLI만 설치하려면 `scripts/install-codex-cli.sh --force`를 사용합니다. 이 CLI installer는 Codex global hook을 수정하지 않습니다. `codex-new`는 현재 `codex-new [PROJECT_DIR] [--name SESSION] [--attach] [--json] [--no-check] [-- CODEX_ARGS...]` 형태이며 기본 모드에서는 bridge-owned tmux 세션 안에서 순정 `codex --dangerously-bypass-approvals-and-sandbox`를 실행합니다. reasoning effort는 모델 인자가 아니라 `CODEX_EFFORT=high|xhigh|medium` env로만 설정합니다. Hermes는 새 visible session을 만들 때 raw `codex` 직접 실행 대신 `codex-new`를 사용하고, 기본값처럼 `--tmux`/`--direct`/hook disable 옵션을 덧붙이지 않습니다.
...
`codex-send --discord-approval`은 `POST /sessions/:id/commands`에 `approvalGate: "discord-hermes-codex-send"`를 붙입니다. bridge는 tmux dispatch 전에 `kind: "codex-send-approval"` structured question을 등록하고 `202 approval-pending`과 `component_actions`/`discord_components`를 반환합니다. 이 JSON은 bridge/API contract일 뿐 Hermes Gateway가 terminal-tool 결과에서 자동으로 버튼을 렌더링하지 않습니다. Hermes agent가 `clarify`/AskUserQuestion을 호출해야 Discord native 카드가 뜹니다. 선택 후 Hermes는 `codex-send --session <id> --answer-approval send|reject --question-id <questionId>` 또는 추가수정 재정제를 사용해 `/sessions/:id/question-answers`에 제출합니다. 전송은 bridge decision log의 `send_claimed` marker 후 exact-once로 dispatch되며, 거절/추가수정은 tmux로 보내지지 않습니다.

Deep Interview 같은 구조화 질문은 tmux 방향키 대신 bridge structured question/answer endpoints를 사용합니다. 먼저 bridge/Codex 쪽 canonical question을 등록한 뒤 answer를 제출해야 합니다.
...
브리지는 외부 session launcher를 필수 의존성으로 사용하지 않습니다.

- 세션 lifecycle: bridge-owned `session-history.jsonl`
- 응답 원문: Codex JSONL session logs
