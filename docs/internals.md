# Internals and risk notes — hermes-codex-notify

이 문서는 `docs/operations.md`에서 뺀 상세 동작과 refactor 후 확인할 edge case를 모읍니다. 평소 운영자는 operations guide만 보면 됩니다.

## Canonical runtime evidence

`hermes-codex-notify`는 bridge-owned lifecycle record와 로컬 Codex JSONL/log를 canonical runtime evidence로 봅니다. tmux list, delivery cursor, event index, channel map은 재생성 가능한 운영 상태입니다. 서로 다른 session을 억지로 하나로 합쳐 두 번째 truth를 만들지 않는 것이 우선입니다.

## Session reconciliation

Codex가 남기는 `session_meta.cwd` 또는 native hook `cwd`가 프로젝트 root와 다른 경우가 있습니다. 이때 bridge는 다음 원칙을 지킵니다.

- visible bridge session의 canonical root/run root에 붙은 Codex log만 같은 session의 `CommandSubmitted`/`FinalAnswer`로 취급합니다.
- root가 다른 native thread를 project session으로 re-label하지 않습니다.
- session이 보이지만 답변 알림이 없다면 `/sessions/:id/state`, `/sessions/:id/events`, `sessionLogPath`, `associatedCodexLogs`를 먼저 확인합니다.
- 필요한 경우 실제 hook cwd root를 `BRIDGE_PROJECT_ROOTS`에 추가해 별도 native session으로 관측합니다. 이 방법은 기존 visible session으로 합치는 기능이 아닙니다.
...
Discord-originated reply dispatch는 `skills/codex-send/SKILL.md`가 prompt refinement SSoT입니다.
...
`codex-send --discord-approval`은 bridge-owned approval question을 만들 뿐입니다. Discord 버튼/선택 UI는 Hermes `clarify`/AskUserQuestion 렌더링이 별도로 필요합니다.
...
Codex lifecycle 전환과 rename 이후 특히 아래를 봅니다.

- `codex-new`가 model을 직접 지정하지 않고 `CODEX_EFFORT`만 반영하는지.
- helper CLI가 raw `codex` 직접 실행 경로보다 bridge lifecycle record를 먼저 남기는지.
...
- install script, systemd service, skill path, webhook subscription이 모두 `hermes-codex-notify` 이름을 쓰는지.
...
bash -n bin/codex-new bin/codex-send bin/codex-kill scripts/*.sh bin/install.sh
npm test
git grep -n '<legacy-name-or-private-secret-pattern>' -- .
```

`npm test`는 현재 node test suite의 authoritative check입니다. skipped tests가 있으면 test output의 skip reason을 보존하고, pass로 보고할 때 skipped 개수도 같이 적습니다.
