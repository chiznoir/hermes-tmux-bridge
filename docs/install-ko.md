...
> bridge와 Hermes/Discord 알림만 설치하는 최소 설치는 `docs/bridge-hermes-only-install-ko.md`를 사용하세요.
...
- `tmux`: visible Codex session과 `codex-new` / `codex-kill`에 필요합니다.
- `curl`: health check, 설치 검증, helper CLI의 bridge HTTP 호출에 필요합니다.
- `jq`: bridge JSON을 읽거나 JSON payload를 만드는 helper CLI에 필요합니다. 특히 `codex-send`와 `codex-kill`은 `jq`가 없으면 실행을 중단하므로, helper CLI를 `PATH`에 설치하는 모드에서는 필수로 봅니다.
...
`~/.config/systemd/user/hermes-codex-bridge.service`에 설치됩니다. 확인/재시작도
...
- `BRIDGE_STATE_ROOT=~/.local/state/hermes-codex-bridge` user service 기준
...
- `CODEX_EFFORT=high` / `medium` / `xhigh` — `codex-new`가 시작하는 Codex reasoning effort를 바꿀 때만 설정합니다.
