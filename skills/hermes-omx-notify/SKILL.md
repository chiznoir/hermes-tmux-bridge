...
```bash
OMX_BRIDGE_URL=${OMX_BRIDGE_URL:-http://127.0.0.1:3037}
```

Read-only inspection examples:

```bash
curl -fsS "$OMX_BRIDGE_URL/health"
curl -fsS "$OMX_BRIDGE_URL/sessions?activity=false&limit=50"
curl -fsS "$OMX_BRIDGE_URL/sessions/$SESSION_ID/state"
curl -fsS "$OMX_BRIDGE_URL/sessions/$SESSION_ID/idle/latest"
curl -fsS "$OMX_BRIDGE_URL/sessions/$SESSION_ID/events"
